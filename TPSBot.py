...
def getCal(url="https://monemploidutemps.unistra.fr/api/calendar/0ccba05c-6cbf-4108-90e2-865ea0ddd0c7/export"):
    ...
    for evt in events:
        ...
        cours["creneau"] = str(dtstart[3]+2) + "h" + str(dtstart[4]) + "0"*(2-len(str(dtstart[4]))) + " - " + str(dtend[3]+2) + "h" + str(dtend[4]) + "0"*(2-len(str(dtend[4])))
...
def demain(j,m,a=-1):
    ...
    if j == 28 and m == 2: # cas part: février
        if a == -1:
            ...
            if est_bissextile(a): return 29, 2, a
            else: return 1, 3, a
...
def run():
    ...
    async def on_message(message):
        ...
        if message.content == "!prochain" or message.content == "!next":
        ...
        if message.content == "!demain":
            ...
            await message.channel.send(to_send)
        if message.content[:7] == "!cours ": # !date jj/mm
            date = message.content[7:].split("/")
            if len(date) != 2:
